...
from mcp_rss_agent.llm.summarize import refine_items


async def handle_message(msg: MCPMessage) -> MCPMessage:
    if msg.action != "hot_news":
        return MCPMessage(id=msg.id, type="error", action=msg.action, payload={"error": "unsupported action"}, ts=datetime.utcnow())

    items = await fetch_feeds()
    items = await refine_items(items)
    payload = HotNewsPayload(items=items)
    return MCPMessage(id=msg.id, type="response", action="hot_news", payload=payload.model_dump(mode="json"), ts=datetime.utcnow())
